...
class CountDown:
    def __init__(self, root):
        self.window = root
        self.window.geometry("310x400+0+0")
        ...
        # inside this frame
        self.button_frame = Frame(self.window, bg="#000", \
        width=240, height=40)
        self.button_frame.place(x=60, y=300)
        # This frame is used to show the countdown time label
        self.time_frame = Frame(self.window, bg="#000", \
        width=380, height=120).place(x=10, y=150)

        # Tkinter Labels
        time_label = Label(self.window, text="Set Timer", 
        font=("magneto 20"), bg='#000',fg='#ea3548')
        time_label.place(x=70,y=15)

        hour_label = Label(self.window, text="  Hour  ", 
        font=("Mistral",10), bg='yellow')
        hour_label.place(x=40, y=90)

        minute_label = Label(self.window, text=" Minute ", 
        font=("Mistral",10), bg='yellow')
        minute_label.place(x=140, y=90)

        second_label = Label(self.window, text=" Second ", 
        font=("Mistral",10), bg='yellow')
        second_label.place(x=240, y=90)
        # ===========================================

        # Tkinter Comboboxes
        # Combobox for hours
        self.hour = IntVar()
        self.hour_combobox = ttk.Combobox(self.window, width=4, 
        height=15, textvariable=self.hour, 
        font=("ErasDemiITC",15))
        self.hour_combobox['values'] = hour_list
        self.hour_combobox.current(0)
        self.hour_combobox.place(x=30,y=120)

        # Combobox for minutes
        self.minute = IntVar()
        self.minute_combobox = ttk.Combobox(self.window, width=4, 
        height=15, textvariable=self.minute, 
        font=("ErasDemiITC",15))
        self.minute_combobox['values'] = min_sec_list
        self.minute_combobox.current(0)
        self.minute_combobox.place(x=130,y=120)

        # Combobox for secs
        self.second = IntVar()
        self.second_combobox = ttk.Combobox(self.window, width=4, 
        height=15, textvariable=self.second, 
        font=("ErasDemiITC",15))
        self.second_combobox['values'] = min_sec_list
        self.second_combobox.current(0)
        self.second_combobox.place(x=230,y=120)
        # ===========================================

        # Tkinter Buttons
        # Cancel button
        cancel_button = Button(self.window, text='Cancel', 
        font=("Mistral 15 bold"), bg="#ea3548", fg="#fff", width =10, height=1, command=self.Cancel)
        cancel_button.place(x=30, y=250)
        ...
        font=("Mistral 15 bold"), bg="#ea3548", fg="#fff", width =10, height=1, command=self.Get_Time)
        set_button.place(x=190, y=250)
    ...
    def Get_Time(self):
        ...
        font=("magneto 15"), 
        bg = '#000', fg = '#fff')
        self.time_display.place(x=40, y=180)
        ...
        try:
            ...
            if s == 0 and m == 0 and h == 0:
                ...
                font=("Mistral 15 bold"), bg="#ea3548", fg="#fff", width=10, height=1,
                command=self.Threading)
                start_button.place(x=7, y=0)
    ...
    def pause_time(self):
        ...
        self.time_display.config(text=f"Countdown - {hours}:{mins}:{secs}")
    ...
    def start_time(self):
        ...
        while self.time_left > 0:
           ...
           #showing a message after complition of the countdown
            if self.time_left <= 0:
                messagebox.showinfo('Time Over','Press Enter : Countdown is over !')
